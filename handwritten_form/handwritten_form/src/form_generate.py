...
def generate_form(form_name, fields):
    print(form_name,fields)
    file_loc = os.path.join(os.path.join(os.getcwd(),'data'),form_name)
    print(file_loc)
    create_image(file_loc, 2480, 3508) # A4 size: 2480x3508
    y_axis_lines = get_lines_position(file_loc, list(fields.values()))
    draw_lines(file_loc, y_axis_lines)
    put_text_in_image(file_loc, list(fields.keys()), y_axis_lines)
